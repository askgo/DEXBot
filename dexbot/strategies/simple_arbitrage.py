...
class Strategy(StrategyBase):
    """ Simple Arbitrage strategy
        ...
        This worker will place a buy or sell order for an asset and update so that arbitrage can be achieved between
        Bitshares DEX and selected CEX exchange. 
    """

    @classmethod
    def configure(cls, return_base_config=True):
        return SimpleArbConfig.configure(return_base_config)

    @classmethod
    def configure_details(cls, include_default_tabs=True):
        return SimpleArbConfig.configure_details(include_default_tabs)
