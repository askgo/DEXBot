...
def get_strategy_tag(strategy_class, arb=False):
...
def configure_worker(whiptail, worker_config, bitshares_instance, arb=False):
    """ Single worker configurator

        :param whiptail.Whiptail whiptail: instance of Whiptail or NoWhiptail
        :param collections.OrderedDict worker_config: the config dictionary for this worker
        :param bitshares.BitShares bitshares_instance: an instance of BitShares class
        :param arb: a boolean for which set of strategies to use. Arb or Regular
    """
    ...
    if arb is True:
        MY_STRATEGIES = ARB_STRATEGIES
        default_strategy = worker_config.get('module', 'dexbot.strategies.simple_arbitrage')
    ...
    if editing and default_strategy != get_strategy_tag(worker_config['module'], arb):
        new_worker_config = {}
...
def configure_dexbot(config, ctx, arb):
    """ Main `cli configure` entrypoint
        ...
        :param dexbot.config.Config config: dexbot config
        Todo: The cli configure needs to be refactored in order to allow
        for logic branching Whiptail does not have if/then logic.
    """
    # feed in arb config is true or false
...
def add_account(whiptail, bitshares_instance):
    ...
    account = whiptail.prompt("Your Bitshares DEX Account Name")
    private_key = whiptail.prompt("Your Bitshares Account Private Key", password=True)

    if not validator.validate_account_name(account):
        whiptail.alert("Bitshares Account name does not exist.")
        return False
    if not validator.validate_private_key(account, private_key):
        whiptail.alert("Bitshares Account Private key is invalid")
        return False
    if private_key and not validator.validate_private_key_type(account, private_key):
        whiptail.alert("Please use Bitshares active private key.")
        return False

    # User can supply empty private key if it was added earlier
    if private_key:
        validator.add_private_key(private_key)
        whiptail.alert(" Bitshares Private Key added successfully.")
